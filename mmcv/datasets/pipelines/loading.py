...
class LoadAnnotations3D(LoadAnnotations):
    ...
    def _load_light_state(self, results):
        ...
        traffic_state = np.ones([results['gt_labels_3d'].shape[0],2],dtype=np.int64)*-1  # [UQ] np.long→np.int64 for NumPy 2.0+
        traffic_state_mask = np.zeros([results['gt_labels_3d'].shape[0]],dtype=np.bool_)  # [UQ] np.bool→np.bool_ for NumPy 2.0+
        ...
        for i,x in enumerate(raw_bbox_anno):
            if x['class'] == 'traffic_light':
                ...
                traffic_state[gt_ids.index(id),1] = np.int64(x['affects_ego'])  # [UQ] np.long→np.int64
