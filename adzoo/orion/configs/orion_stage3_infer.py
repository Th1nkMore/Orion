...
model = dict(
    ...
    use_uncertainty_l2 = False,  # [UQ] FiLM L2 at VAE level (disabled until trained)
    use_bev_uncertainty = False,  # [UQ] BEV spatial cost for mode selection (disabled until validated)
    bev_lambda = 0.5,             # [UQ] BEV cost injection strength
    img_backbone=dict(
        type='EVAViT',
        img_size=640, 
        patch_size=16,
    ...
    pts_bbox_head=dict(
        ...
        use_uncertainty=True,  # [UQ] Phase 2: enable UQ estimation
        uq_checkpoint='checkpoints/density_uq/best.pt',  # [UQ] normal-density model
