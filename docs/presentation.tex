...
$$u_i = \sum_j \underbrace{a_{ij}}_{\text{QT-Former attn}} \cdot (1 - q_j), \quad [B, 900] \to [B,30,30]$$

\vspace{0.3em}
\textbf{Step 3}：轨迹代价积分
$$c_m = \text{mean}_t \; \text{bilinear}(U, \text{wp}_{m,t})$$

\vspace{0.3em}
\textbf{Step 4}：调整 mode 选择（\alert{仅学 $\lambda$，1个标量}）
$$\hat{\ell}_m = \ell_m - \lambda \cdot s \cdot c_m$$
\end{column}
...
第 3--4 周 & 6 组完整闭环评估 & \textcolor{mAlertRed}{A100} & 主实验表 \\
\midrule
...
\vspace{0.3em}
\small
\begin{tabular}{p{0.3cm}p{1.6cm}p{1cm}p{1.2cm}}
\toprule
组 & 配置 & SG-FiLM & BEV Cost \\
\midrule
A & Baseline & --- & --- \\
B & FiLM-v3 & $\times$ & --- \\
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item 轨迹模式选择时主动规避感知可靠性低的区域
        \item 核心计算无需 LLM，16GB 本地可运行
    \end{itemize}

    \vspace{0.3em}
    \item \textbf{即插即用}：参数增量 $<$ 0.5M，适用于任意冻结的端到端驾驶模型
\end{enumerate}
