...
The v1 eligible GT set is car, van, truck, bicycle and pedestrian, plus a
traffic light only when its loader state is valid and `affects_ego=true`.
Traffic signs, cones and `others` are excluded from the primary target. Every
filter must be applied on one shared object axis across boxes, classes, future
attributes, traffic state, actor IDs and projected support.
...
For the first diagnostic pilot, the preregistered matching policy uses decoded
score `>=0.50` and center distance `<=4 m`, with a `2 m` sensitivity report.
These are auditable heuristic gates, not claimed optimal calibration. They
must not be tuned on validation/held-out outcomes; later adjustment is fitted
only on the route-disjoint calibration split and creates a new policy ID.
...
In exporter v1, “valid” means the annotation loader marks a traffic light and
its privileged `affects_ego` flag is true. ORION's first three traffic logits
use sigmoid focal-loss semantics for the state label. The fourth affects-ego
logit is retained for audit but is not mixed into `e_tl_j`; a joint state /
affect target would be a separately versioned component.
