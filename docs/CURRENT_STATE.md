...
Last audited: 2026-09-03 (Asia/Shanghai)
...
### 3.6 U-to-language alignment pilot completed with soft failures
...
Exactly one Job `1131873` (`s2l_v15_u`) ran on `gpu1` with one A800, two CPU
cores and 192 GB host memory. It started `2026-09-02T19:55:35+08:00` and ended
`2026-09-03T04:50:09+08:00` (`08:54:34`, exit `0:0`, state `COMPLETED`). The
trainer recorded 720/720 finite optimizer steps. Report status is
`bounded_u_language_alignment_completed_with_soft_failures`;
`soft_diagnostics_passed` is `false`. Reconstruction and the zero-U presence
anchor are solved; nonzero spatial U, especially `U_VIEW`, is not identifiable.

| Diagnostic | Before | After | Soft gate |
|---|---:|---:|---|
| Mean reconstruction loss | 0.178045 | 0.019559 | improved (pass) |
| Mean zero-anchor loss | 0.177923 | 5.2887e-5 | improved (pass) |
| Constrained `U_PRESENT` acc | 1.00 | 1.00 | --- |
| Constrained `U_TREND` acc | 0.39 | 0.49 | --- |
| Constrained `U_REGION` acc | 0.26 | 0.44 | --- |
| Constrained `U_LEVEL` acc | 0.58 | 0.41 | worse |
| Constrained `U_COMPONENT` acc | 0.58 | 0.36 | worse |
| Constrained `U_VIEW` acc | 0.18 | 0.29 | --- |
| `U_VIEW` balanced accuracy | 0.29 | 0.29 | `>= 0.45` fail |
| Nonzero acc excluding presence | 0.296 | 0.278 | `>= 0.50` fail; not improved vs v14 |
| Changed-field exact response | 0.218 | 0.257 | `>= 0.40` fail |
| Free generation parseable | n/a | 0 / 24 | --- |

Failed official soft diagnostics: nonzero U semantics vs v14 majority baseline,
nonzero improvement, changed-field response, and view-balanced accuracy.
`U_PRESENT` remains `1.00` on 720 field decisions / 20 groups. Some fields are
worse than the v14-initialized before snapshot (`U_COMPONENT` 0.58->0.36,
`U_LEVEL` 0.58->0.41). Locks stay closed: no extra epochs, no automatic retry,
no formal Stage2-L, no Stage2-P, no closed loop, no locked-test. This is not a
safety result. The terminal record is
`configs/scenario_factory/amendments/20260903_stage2l_v15_u_language_alignment_terminal_v1.json`.
The prior hash-bound launch record remains
...
**Current decision (2026-09-03):** the v15 U-to-language alignment pilot in
Section 3.6 is complete with soft failures. Reconstruction and zero-U presence
are solved; nonzero spatial U, especially `U_VIEW`, is not identifiable. Do
not submit another training job. There is no automatic retry, extra epoch,
task-relevance training, formal Stage2-L, Stage2-P, closed loop or locked-test
unlock.
