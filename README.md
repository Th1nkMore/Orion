...
The largest immediate gap is **held-out contextual task-relevance localization across event/view and support-mode patterns**. A CPU audit rebuilt all 80 geometry targets and their 10x10 trainer tensors at zero error, ruling out stale lineage or a deterministic camera/projection mismatch. The train foreground objective is nevertheless front-dominant (`65.76%` on `CAM_FRONT`, about `4–5%` on each sampled side view, `0%` on `CAM_BACK_RIGHT`); held-out front-left/front-right recall is zero, and combined route-plus-actor controls generalize much worse than route-only controls. Until R is view-balanced and independently covered, the language bridge cannot receive a valid held-out U-to-task signal. See [docs/CURRENT_STATE.md](./docs/CURRENT_STATE.md).

The original v11 run (Job `1120666`) remains recorded as a 10x10 consumer-grid control-contract failure. Its separately versioned v11.1 replacement fixes that defect exactly; Job `1120954` then validly localized the failure to frozen R (`0.8667` train versus `0.55` dev) before language optimization. The calibrated v12 within-group view objective now passes its CPU integrity checks, but it cannot repair the event-level absence: `CAM_BACK_RIGHT` still has zero positive train groups and only two independent train events contain `CAM_FRONT_RIGHT`. Route177/201/208 cannot directly fill this gap under their frozen geometry/runtime/liveness dispositions. The next permitted work is a pre-outcome, train-only coverage-repair pool and another CPU coverage gate—not a GPU smoke. Language, Stage2-P, learned-U closed loop and benchmark work remain unauthorized.

The coverage-repair audit has now exhausted the cheap existing-data path. Route196 contributes five geometry-valid train-only frames and raises front-right independent-event coverage, but not back-right. A scan of every aligned raw frame in all 14 accepted train event identities found zero `CAM_BACK_RIGHT` relevance support. The preregistered fallback therefore froze Route167 (`YieldToEmergencyVehicle`) for one outcome-blind `clean_off` collection under a non-held-out `train_coverage_repair` split. Its first Job `1121242` is an invalid technical launch: a relative route path failed before RouteIndexer loaded the XML, so it produced no model or safety evidence. The submitter now exports an absolute route directory and has a passing regression test; the separately attested `retry1` replacement is Job `1121244`, submitted pending under an exhausted one-job authorization. This remains data acquisition only, and no R/U/language/planning optimization is unlocked.
