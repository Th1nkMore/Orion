...
The 17-event v11 engineering dataset passed its stored-tensor audit, and the one authorized run completed normally as Slurm Job `1120666`, but failed closed before language training at step 0. A post-run audit exposed a missing preflight invariant: all 80 on/off U pairs matched on the stored 40x40 grid, but only 7 remained magnitude-matched after the tokenizer's actual 10x10 pooling. This is an engineering control-contract failure, not a valid new R comparison. A separately versioned consumer-grid-matched dataset is required; the frozen R checkpoint retains its failed held-out release status, and formal Stage2-L, Stage2-P, learned-U closed loop and benchmark claims remain locked.
