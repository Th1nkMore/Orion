...
> **Current-state authority (2026-09-02):** [docs/CURRENT_STATE.md](./docs/CURRENT_STATE.md) records the active architecture, exact completed gates, failed gates, open blockers, and next executable milestone. Machine-readable experiment authority remains in `configs/scenario_factory/` and hash-bound result records in `results/scenario_factory/`.
>
...
That repair is now preflight-complete and submitted as the single bounded v15 Job `1131873`. It keeps the Stage-1 observation estimator frozen, removes R/bridge/task/planning inputs, and jointly aligns the existing U-tokenizer projection with ORION LoRA. Its 720-step schedule balances all six fields and every canonical value; evaluation covers all 120 dev U states with all-candidate decoding. The job is currently `PENDING (Priority)`. No result or downstream unlock is implied by submission.
