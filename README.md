...
The 17-event v11 engineering dataset has passed its metadata and full U-tensor identifiability audit (80 matched groups / 1,600 QA records). The shared-R/K/QA runtime and bridge-only trainer have also passed a remote CPU/lineage preflight. The one authorized 40-step engineering run was submitted as Slurm Job `1120666` and started on `gpu4`; no v11 model result exists yet. The frozen R checkpoint still retains its failed held-out release status, and formal Stage2-L, Stage2-P, learned-U closed loop and benchmark claims remain locked.
