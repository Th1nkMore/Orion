...
The original v11 run (Job `1120666`) remains recorded as a 10x10 consumer-grid control-contract failure. Its separately versioned v11.1 replacement fixes that defect exactly; Job `1120954` then validly localized the failure to frozen R (`0.8667` train versus `0.55` dev) before language optimization. The next permitted work is CPU-first implementation of a calibrated view-balanced R objective plus geometry-only right-view event coverage, followed by at most one separately authorized R-only smoke. It does not authorize language, Stage2-P, learned-U closed loop or benchmark work.
