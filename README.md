...
The largest immediate gap is now **held-out contextual task-relevance localization across event/view patterns**. The exact consumer-grid v11.1 controls show that frozen R supports train on/off ordering (`52/60`) but only `11/20` held-out groups; all ten held-out `CAM_FRONT` cases pass, while left-side and no-effective-risk-region cases pass `0/9`. Until R can bind those regions reliably, the language bridge cannot receive a valid held-out U-to-task signal. Learned Stage-1 U external validity remains a separate later gate. See [docs/CURRENT_STATE.md](./docs/CURRENT_STATE.md).

The original v11 run (Job `1120666`) remains recorded as a 10x10 consumer-grid control-contract failure. Its separately versioned v11.1 replacement fixes that defect exactly: all 80 pairs match peak, mass and support count after pooling. The one authorized replacement (Job `1120954`) completed normally but stopped at step 0 because train passed the controlled-U gate (`0.8667`) and dev failed (`0.55 < 0.8`). This validly localizes the next repair to R supervision/view binding/event generalization; it does not test the language bridge. Formal Stage2-L, Stage2-P, learned-U closed loop and benchmark claims remain locked.
