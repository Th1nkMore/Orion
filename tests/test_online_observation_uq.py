...
def test_pool_observation_evidence_grid_is_json_safe_and_view_specific():
    score = torch.zeros(1, 2, 4, 6, 3)
    score[:, 1, :, :, :] = 3.0
    first = pool_observation_evidence_grid(score, view_index=0, grid_size=2)
    second = pool_observation_evidence_grid(score, view_index=1, grid_size=2)
    assert first == ((0.0, 0.0), (0.0, 0.0))
    assert second == ((3.0, 3.0), (3.0, 3.0))
    json.dumps(second)


def test_pool_observation_evidence_grid_rejects_invalid_view_and_size():
    score = torch.zeros(1, 2, 4, 6, 3)
    with pytest.raises(ValueError, match="view index"):
        pool_observation_evidence_grid(score, view_index=2)
    with pytest.raises(ValueError, match="grid_size"):
        pool_observation_evidence_grid(score, grid_size=0)


def test_pool_observation_evidence_grids_preserves_view_axis():
    score = torch.zeros(1, 3, 4, 6, 3)
    score[:, 1] = 2.0
    score[:, 2] = 4.0
    grids = pool_observation_evidence_grids(score, grid_size=2)
    assert len(grids) == 3
    assert grids[0][0][0] == 0.0
    assert grids[1][0][0] == 2.0
    assert grids[2][0][0] == 4.0
