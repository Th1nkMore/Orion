...
protocol="${STAGE2L_TRAINING_PROTOCOL:-${project_root}/configs/scenario_factory/stage2l_training_v4_task_risk_bridge.json}"
amendment="${LAUNCH_AMENDMENT:-${project_root}/configs/scenario_factory/amendments/20260829_stage2l_route196_task_risk_bridge_launch_v4.json}"
output_root="${OUTPUT_ROOT:-${asset_root}/scenario_factory/stage2l_smokes/route196_task_risk_bridge_v1_240/training}"
log_root="${LOG_ROOT:-${asset_root}/logs/stage2l_route196_task_risk_bridge_v1_240}"
...
if [[ -e "${output_root}" ]] && find "${output_root}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite bridge smoke output: ${output_root}" >&2
  exit 1
fi
...
expected = {
    ...
    "trainer_sha256": sha(root / "scripts/train_stage2l_route196_bridge_smoke.py"),
    ...
    "submit_wrapper_sha256": sha(root / "scripts/submit_stage2l_route196_bridge_smoke.sh"),
}
if amendment.get("schema") != "orion.scenario_factory.amendment.v1":
    raise SystemExit("invalid bridge launch amendment schema")
if locks.get(key) is not True or locks.get("stage2l_pilot_training_allowed") is not False:
    raise SystemExit("bridge launch is not active while the pilot remains locked")
if locks.get("stage2p_allowed") is not False:
    raise SystemExit("bridge smoke may not authorize Stage2-P")
if int(authorized.get("maximum_submissions", 0)) != 1:
    raise SystemExit("bridge amendment is not a single-run authorization")
if Path(authorized.get("output_root", "")).resolve() != output_root.resolve():
    raise SystemExit("bridge output root differs from the amendment")
if any(validated.get(name) != value for name, value in expected.items()):
    raise SystemExit("bridge source/protocol hash differs from the amendment")
print("BRIDGE_SINGLE_RUN_AUTHORIZED=1")
PY

train_parts=(
  env "PYTHONPATH=${runtime_pythonpath}" "IS_BENCH2DRIVE=True"
  "${python_bin}" "${project_root}/scripts/train_stage2l_route196_bridge_smoke.py"
  --config "${config}"
...
mkdir -p "${log_root}"
job_id="$("${sbatch_args[@]}")"
echo "STAGE2L_ROUTE196_BRIDGE_JOB_ID=${job_id}"
