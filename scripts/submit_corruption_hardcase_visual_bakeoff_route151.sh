...
log_root="${asset_root}/scenario_factory/logs/corruption_hardcase_bakeoffs"
job_name="hardcase151_vis"

if [[ -e "${output_root}" ]]; then
  echo "refusing to reuse hard-case bake-off output root: ${output_root}" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active hard-case visual bake-off" >&2
  exit 1
fi
...
wrapped=(
  ...
  bash "${project_root}/scripts/run_corruption_hardcase_visual_bakeoff_route151.sh"
)
