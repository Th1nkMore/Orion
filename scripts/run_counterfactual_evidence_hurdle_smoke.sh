...
run_id="${RUN_ID:-counterfactual_evidence_hurdle_smoke_seed20260827_r1}"
...
config="${project_root}/configs/observation_uq_counterfactual_hurdle_smoke_v1.json"
...
echo "RUN_ID=${run_id}"
echo "SCOPE=route_only_hurdle_architecture_smoke"
echo "EPOCHS=4"
...
"${project_root}/scripts/run_compat_python.sh" \
  ...
  --config "${config}" \
  --output "${output_root}/counterfactual_evidence_hurdle_smoke.pt" \
  --device cuda

sha256sum "${output_root}/counterfactual_evidence_hurdle_smoke.pt" \
  "${output_root}/counterfactual_evidence_hurdle_smoke.report.json" \
  > "${output_root}/artifact_sha256.txt"
echo "COUNTERFACTUAL_EVIDENCE_HURDLE_SMOKE_JOB_OK=1"
