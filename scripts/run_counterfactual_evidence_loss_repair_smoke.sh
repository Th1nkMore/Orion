...
target_root="${asset_root}/observation_uq_v3/runs/counterfactual_evidence_target_audit_seed20260826_r2"
spatial_root="${asset_root}/observation_uq_v3/runs/counterfactual_evidence_spatial_support_seed20260827_r1"
config="${project_root}/configs/observation_uq_counterfactual_loss_repair_smoke_v1.json"
...
echo "RUN_ID=${run_id}"
echo "SCOPE=route_only_loss_repair_smoke"
echo "EPOCHS=4"
...
sha256sum "${config}" "${project_root}/uq_estimator/counterfactual_evidence.py" \
  ...
  "${project_root}/scripts/train_counterfactual_evidence_loss_repair_smoke.py" \
...
"${project_root}/scripts/run_compat_python.sh" \
  ...
  --config "${config}" \
  --output "${output_root}/counterfactual_evidence_loss_repair_smoke.pt" \
  --device cuda

sha256sum "${output_root}/counterfactual_evidence_loss_repair_smoke.pt" \
  "${output_root}/counterfactual_evidence_loss_repair_smoke.report.json" \
  > "${output_root}/artifact_sha256.txt"
echo "COUNTERFACTUAL_EVIDENCE_LOSS_REPAIR_SMOKE_JOB_OK=1"
