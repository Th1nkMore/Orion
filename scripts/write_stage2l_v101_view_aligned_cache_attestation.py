#!/usr/bin/env python3
"""Write a hash-bound submission attestation for the v10.1 cache-only job."""
...
SCHEMA = "orion.stage2l_v101_view_aligned_cache_submission.v1"
...
def main() -> int:
    ...
    if args.output.exists() or not str(args.job_id).isdigit():
        raise ValueError("submission attestation target/job id is invalid")
    protocol = json.loads(args.protocol.read_text(encoding="utf-8"))
    preflight = json.loads(args.preflight.read_text(encoding="utf-8"))
    amendment = json.loads(args.amendment.read_text(encoding="utf-8"))
    validated = {
        "cache_builder_sha256": sha256_file(args.cache_builder.resolve()),
    ...
    if (
        ...
        or amendment.get("authorized_run", {}).get("output")
        != str(args.cache_output.resolve())
    ):
        raise ValueError("submission inputs differ from the frozen lineage")
    value = {
        "schema": SCHEMA,
        "status": "submitted_cache_only_job_attested",
        "job_id": str(args.job_id),
        "remote_log": str(args.remote_log),
        "cache_output": str(args.cache_output.resolve()),
