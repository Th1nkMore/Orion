...
echo "SCOPE=v2_counterfactual_train_target_and_schedule_audit_only"
echo "CONTINUOUS_TARGET_AUDIT=1"
...
"${project_root}/scripts/run_compat_python.sh" \
  ...
  --device cuda --batch-size 2 --quantile 0.95 --response-floor 1e-6

sha256sum "${output_root}/counterfactual_evidence_target_audit.json" \
  > "${output_root}/artifact_sha256.txt"
echo "COUNTERFACTUAL_EVIDENCE_TARGET_AUDIT_V2_JOB_OK=1"
