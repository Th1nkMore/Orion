...
wrapped=(env "PROJECT_ROOT=${project_root}" "ASSET_ROOT=${asset_root}" "RUN_ID=${run_id}" "OUTPUT_ROOT=${output_root}" bash "${project_root}/scripts/run_native_carla_appearance_candidates.sh")
...
sbatch_args=(
  ...
  --time=00:30:00 "${nodelist_args[@]}" --job-name=native_appearance
...
if [[ -e "${output_root}" ]]; then
  echo "refusing to reuse appearance audit output root: ${output_root}" >&2
  exit 1
fi
mkdir -p "${log_root}"
"${sbatch_args[@]}"
