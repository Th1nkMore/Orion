...
run_id="${RUN_ID:-counterfactual_evidence_features_seed20260826_r1}"
...
wrapped=(env "PROJECT_ROOT=${project_root}" "ASSET_ROOT=${asset_root}" "RUN_ID=${run_id}" "OUTPUT_ROOT=${output_root}" bash "${project_root}/scripts/run_counterfactual_evidence_extraction.sh")
printf -v wrapped_command '%q ' "${wrapped[@]}"
sbatch_args=(
  sbatch --parsable "--partition=${partition}" --gres=gpu:1 --cpus-per-task=8
  --mem=160G --time=03:00:00 --job-name=cf_evidence_extract
...
if [[ -e "${output_root}" ]]; then
  echo "refusing to reuse counterfactual extraction output root: ${output_root}" >&2
  exit 1
fi
mkdir -p "${log_root}"
"${sbatch_args[@]}"
