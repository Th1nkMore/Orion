...
protocol="${project_root}/configs/scenario_factory/stage2l_mr1_training_v1.json"
preflight="${asset_root}/scenario_factory/stage2l_smokes/mr1_8event_v1_40/trainer_preflight.json"
amendment="${project_root}/configs/scenario_factory/amendments/20260831_stage2l_mr1_8event_preexperiment_v1.json"
output_parent="${asset_root}/scenario_factory/stage2l_smokes/mr1_8event_v1_40"
...
for prerequisite in \
  ...
  if [[ ! -f "${prerequisite}" ]]; then
    echo "missing MR1 prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
if [[ -e "${output_dir}" ]] && find "${output_dir}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite non-empty MR1 output: ${output_dir}" >&2
  exit 1
fi
if [[ -e "${attestation}" ]]; then
  echo "refusing to overwrite MR1 submission attestation" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active MR1 submission: ${job_name}" >&2
  exit 1
fi
...
job_id="$(sbatch --parsable \
    ...
    --exclude=gpu5 \
    --job-name="${job_name}" \
    --output="${log_root}/mr1_8event_v1_40-%j.out" \
    --export=ALL \
    --wrap "${train_command}")"
...
cancel_unattested_submission() {
  if [[ -n "${unattested_job_id}" ]]; then
    ...
    echo "cancelled unattested MR1 job: ${unattested_job_id}" >&2
  fi
}
trap cancel_unattested_submission EXIT INT TERM

remote_log="${log_root}/mr1_8event_v1_40-${job_id}.out"
