...
wrapped=(env "PROJECT_ROOT=${project_root}" "ASSET_ROOT=${asset_root}" "RUN_ID=${run_id}" bash "${project_root}/scripts/run_counterfactual_evidence_adapter_pilot.sh")
printf -v wrapped_command '%q ' "${wrapped[@]}"
sbatch_args=(
  sbatch --parsable "--partition=${partition}" --gres=gpu:1 --cpus-per-task=4
  --mem=140G --time=03:00:00 --job-name=cf_adapter_pilot
  "--output=${log_root}/${run_id}-%j.out" --export=ALL --wrap "${wrapped_command}"
)
if [[ "${submit}" != "1" ]]; then
  echo "DRY_RUN_ONLY=1"
  echo "RESOURCE_CONTRACT=partition:${partition},gpu:1,cpus:4,mem:140G,time:03:00:00"
  echo "EPOCHS=24"
