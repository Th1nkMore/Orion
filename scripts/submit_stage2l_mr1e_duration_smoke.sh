...
dataset_manifest="${asset_root}/scenario_factory/stage2l_multiroute_smokes/stage2l_mr1_8event_v5_dataset_v1/manifest.json"
protocol="${project_root}/configs/scenario_factory/stage2l_mr1e_80step_duration_v1.json"
preflight="${asset_root}/scenario_factory/stage2l_smokes/mr1e_8event_v1_80/trainer_preflight.json"
amendment="${project_root}/configs/scenario_factory/amendments/20260831_stage2l_mr1e_80step_duration_v1.json"
output_parent="${asset_root}/scenario_factory/stage2l_smokes/mr1e_8event_v1_80"
...
job_name="s2l_mr1_8event"

for prerequisite in \
  "${project_root}/scripts/train_stage2l_mr1_smoke.py" \
  "${project_root}/scripts/write_stage2l_mr1_submission_attestation.py" \
  "${dataset_manifest}" "${protocol}" "${preflight}" "${amendment}"; do
  if [[ ! -f "${prerequisite}" ]]; then
    echo "missing MR1-E prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
if [[ -e "${output_dir}" ]] && find "${output_dir}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite non-empty MR1-E output: ${output_dir}" >&2
  exit 1
fi
if [[ -e "${attestation}" ]]; then
  echo "refusing to overwrite MR1-E submission attestation" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active MR1-E submission: ${job_name}" >&2
  exit 1
fi
...
train_parts=(
  ...
  "${python_bin}" "${project_root}/scripts/train_stage2l_mr1_smoke.py"
...
job_id="$(sbatch --parsable \
    ...
    --exclude=gpu5 \
    --job-name="${job_name}" \
    --output="${log_root}/mr1e_8event_v1_80-%j.out" \
    --export=ALL \
    --wrap "${train_command}")"
...
cancel_unattested_submission() {
  if [[ -n "${unattested_job_id}" ]]; then
    ...
    echo "cancelled unattested MR1-E job: ${unattested_job_id}" >&2
  fi
}
trap cancel_unattested_submission EXIT INT TERM

remote_log="${log_root}/mr1e_8event_v1_80-${job_id}.out"
env "PYTHONPATH=${project_root}" "${python_bin}" \
  ...
  --dataset-manifest "${dataset_manifest}" \
  --trainer-preflight "${preflight}" \
  --trainer "${project_root}/scripts/train_stage2l_mr1_smoke.py" \
  --job-id "${job_id}" \
