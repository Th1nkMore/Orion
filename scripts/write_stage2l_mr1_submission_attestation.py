#!/usr/bin/env python3
"""Write one immutable attestation for the bounded MR1 Slurm submission."""
...
SCHEMA = "orion.stage2l_mr1_submission_attestation.v1"
...
def main() -> int:
    ...
    if args.output.exists():
        raise FileExistsError("refusing to overwrite MR1 attestation")
    ...
    if (
        ...
        or amendment.get("launch_locks", {}).get(
            "stage2l_mr1_bounded_preexperiment_allowed"
        )
        is not True
        ...
        raise ValueError("MR1 launch inputs differ from the amendment")
    ...
    result = {
        ...
        "maximum_optimizer_steps": run["maximum_optimizer_steps"],
