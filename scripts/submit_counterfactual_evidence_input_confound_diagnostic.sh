...
wrapped=(env \
  ...
  bash "${PROJECT_ROOT}/scripts/run_counterfactual_evidence_input_confound_diagnostic.sh")
...
if [[ "${submit}" != 1 ]]; then
  echo "DRY_RUN_ONLY=1"
  echo "RESOURCE_CONTRACT=partition:${PARTITION},gpu:1,cpus:4,mem:64G,time:00:30:00"
  echo "TRAINING=0"
