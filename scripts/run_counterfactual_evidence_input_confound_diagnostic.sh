...
CLEAN_TAIL_RUN_ID="${CLEAN_TAIL_RUN_ID:-counterfactual_evidence_clean_tail_seed20260827_r1}"
RUN_ID="${RUN_ID:-counterfactual_evidence_input_confound_seed20260827_r1}"
...
CLEAN_TAIL_ROOT="${HOME_WORK_ROOT}/observation_uq_v3/runs/${CLEAN_TAIL_RUN_ID}"
OUTPUT_ROOT="${HOME_WORK_ROOT}/observation_uq_v3/runs/${RUN_ID}"
CONFIG_PATH="${CONFIG_PATH:-${PROJECT_ROOT}/configs/observation_uq_counterfactual_input_confound_diagnostic_v1.json}"
...
test -s "${CLEAN_TAIL_ROOT}/counterfactual_evidence_clean_tail.report.json"
test -s "${CONFIG_PATH}"
test ! -e "${OUTPUT_ROOT}"
mkdir -p "${OUTPUT_ROOT}"
...
echo "RUN_ID=${RUN_ID}"
echo "SCOPE=checkpoint_only_input_confound_ablation"
echo "TRAINING=0"
echo "VALIDATION_ROUTES=10"
echo "VARIANTS=original,no_explicit_change_scalar,no_explicit_rms_scalars,no_explicit_scalars,no_view_embedding,no_temporal_branch"
...
sha256sum \
  ...
  "${PROJECT_ROOT}/scripts/diagnose_counterfactual_input_confounds.py" \
...
"${PROJECT_ROOT}/scripts/run_compat_python.sh" \
  ...
  --clean-tail-report "${CLEAN_TAIL_ROOT}/counterfactual_evidence_clean_tail.report.json" \
  --config "${CONFIG_PATH}" \
  --output "${OUTPUT_ROOT}/counterfactual_evidence_input_confound.report.json" \
  --device cuda

sha256sum "${OUTPUT_ROOT}/counterfactual_evidence_input_confound.report.json" \
  > "${OUTPUT_ROOT}/artifact_sha256.txt"
echo "COUNTERFACTUAL_EVIDENCE_INPUT_CONFOUND_JOB_OK=1"
