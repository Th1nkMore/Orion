...
wrapped=(env "PROJECT_ROOT=${project_root}" "ASSET_ROOT=${asset_root}" "RUN_ID=${run_id}" bash "${project_root}/scripts/run_counterfactual_evidence_spatial_support_audit.sh")
printf -v wrapped_command '%q ' "${wrapped[@]}"
sbatch_args=(
  sbatch --parsable "--partition=${partition}" --gres=gpu:1 --cpus-per-task=2
  --mem=120G --time=00:45:00 --job-name=cf_spatial_audit
