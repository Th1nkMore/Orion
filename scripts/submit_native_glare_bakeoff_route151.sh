...
log_root="${asset_root}/scenario_factory/logs/glare_bakeoffs"
job_name="glare151_native"

if [[ -e "${output_root}" ]]; then
  echo "refusing to reuse native glare output root: ${output_root}" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active native glare bake-off" >&2
  exit 1
fi
...
wrapped=(
  ...
  bash "${project_root}/scripts/run_native_glare_bakeoff_route151.sh"
)
printf -v wrapped_command '%q ' "${wrapped[@]}"
sbatch_args=(
  sbatch --parsable --partition=Nvidia_A800 --gres=gpu:1
  --cpus-per-task=2 --mem=16G --time=02:00:00 --exclude=gpu5
  --job-name="${job_name}"
...
if [[ "${submit}" != "1" ]]; then
  echo "DRY_RUN_ONLY=1"
  echo "RUN_ID=${run_id}"
  echo "RESOURCE_CONTRACT=partition:Nvidia_A800,gpu:1,cpus:2,mem:16G,time:02:00:00"
  echo "ORION_LOAD=0"
