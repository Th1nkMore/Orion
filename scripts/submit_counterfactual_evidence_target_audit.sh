...
dependency_job="${DEPENDENCY_JOB:-1067228}"

wrapped=(env "PROJECT_ROOT=${project_root}" "ASSET_ROOT=${asset_root}" "AUDIT_RUN_ID=${audit_run_id}" bash "${project_root}/scripts/run_counterfactual_evidence_target_audit.sh")
printf -v wrapped_command '%q ' "${wrapped[@]}"
sbatch_args=(
  sbatch --parsable "--partition=${partition}" --gres=gpu:1 --cpus-per-task=2
  --mem=120G --time=00:45:00 --job-name=cf_target_audit
  "--dependency=afterok:${dependency_job}"
  "--output=${log_root}/${audit_run_id}-%j.out" --export=ALL
