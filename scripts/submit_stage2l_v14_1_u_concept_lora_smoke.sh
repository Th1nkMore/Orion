...
protocol="${project_root}/configs/scenario_factory/stage2l_v14_1_u_concept_lora_runtime_fix_v1.json"
...
run_name="v14_1_u_concept_lora_17event_200_v1"
job_name="s2l_v141_u"
...
for prerequisite in \
  ...
  if [[ ! -f "${prerequisite}" ]]; then
    echo "missing v14.1 U-concept prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
if [[ -e "${output_root}" ]] && find "${output_root}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite non-empty v14.1 output: ${output_root}" >&2
  exit 1
fi
if [[ -e "${preflight}" ]]; then
  echo "refusing to overwrite existing v14.1 preflight: ${preflight}" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active v14.1 job: ${job_name}" >&2
  exit 1
fi
