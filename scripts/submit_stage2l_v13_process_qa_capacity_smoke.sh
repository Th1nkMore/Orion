...
protocol="${project_root}/configs/scenario_factory/stage2l_v13_process_qa_capacity_protocol_v1.json"
launch="${project_root}/configs/scenario_factory/amendments/20260902_stage2l_v13_direct_u_r_process_qa_launch_v1.json"
...
if [[ "${arm}" == "lora" ]]; then
  run_name="v13_process_qa_lora_17event_200_v1"
  job_name="s2l_v13_lora"
else
  run_name="v13_process_qa_partial4_17event_200_v1"
  job_name="s2l_v13_p4"
fi
...
for prerequisite in \
  ...
  if [[ ! -f "${prerequisite}" ]]; then
    echo "missing v13 prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
if [[ -e "${output_root}" ]] && find "${output_root}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite non-empty v13 output: ${output_root}" >&2
  exit 1
fi
if [[ -e "${attestation}" ]]; then
  echo "refusing to overwrite v13 submission attestation" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active v13 arm: ${job_name}" >&2
  exit 1
fi
...
cancel_unattested_submission() {
  if [[ -n "${unattested_job_id}" ]]; then
    ...
    echo "cancelled unattested v13 job: ${unattested_job_id}" >&2
  fi
}
