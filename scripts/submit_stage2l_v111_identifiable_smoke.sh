...
base_root="${asset_root}/scenario_factory/stage2l_smokes/v11_1_identifiable_17event_40_v1"
...
consumer_grid_audit="${data_root}/consumer_grid_audit.json"
dataset_upgrade_report="${data_root}/upgrade_report.json"
...
protocol="${project_root}/configs/scenario_factory/stage2l_v11_1_bounded_identifiability_smoke_protocol_v1.json"
amendment="${project_root}/configs/scenario_factory/amendments/20260901_stage2l_v111_identifiability_smoke_launch_v1.json"
...
attester="${project_root}/scripts/write_stage2l_v111_submission_attestation.py"
...
for prerequisite in \
  ...
  "${consumer_grid_audit}" "${dataset_upgrade_report}" \
  ...
  if [[ ! -f "${prerequisite}" ]]; then
    echo "missing v11.1 prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
if [[ -e "${output_root}" ]] && find "${output_root}" -mindepth 1 -print -quit | grep -q .; then
  echo "refusing to overwrite non-empty v11.1 output" >&2
  exit 1
fi
if [[ -e "${attestation}" ]]; then
  echo "refusing to overwrite v11.1 submission attestation" >&2
  exit 1
fi
if squeue -h -u "${USER}" -n "${job_name}" | grep -q .; then
  echo "refusing duplicate active v11.1 submission" >&2
  exit 1
fi
...
job_id="$(sbatch --parsable \
  ...
  --exclude=gpu5 \
  --job-name="${job_name}" \
  --output="${log_root}/v11_1_identifiable_17event_40_v1-%j.out" \
  --export=ALL \
  --wrap "${train_command}")"
...
cancel_unattested_submission() {
  if [[ -n "${unattested_job_id}" ]]; then
    ...
    echo "cancelled unattested v11.1 job: ${unattested_job_id}" >&2
  fi
}
trap cancel_unattested_submission EXIT INT TERM

remote_log="${log_root}/v11_1_identifiable_17event_40_v1-${job_id}.out"
env "PYTHONPATH=${project_root}" "${python_bin}" "${attester}" \
  ...
  --v11-records "${v11_records}" \
  --dataset-audit-report "${dataset_audit}" \
  --consumer-grid-audit "${consumer_grid_audit}" \
  --dataset-upgrade-report "${dataset_upgrade_report}" \
  --view-feature-cache "${view_feature_cache}" \
  --u-tokenizer-checkpoint "${u_tokenizer}" \
  --v101-checkpoint "${v101_checkpoint}" \
  --v101-report "${v101_report}" \
