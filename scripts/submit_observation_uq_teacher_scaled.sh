...
run_id="${RUN_ID:-observation_uq_teacher_scaled_$(date +%Y%m%dT%H%M%S)}"
...
wrapped_parts=(
  ...
  bash
  "${project_root}/scripts/run_observation_uq_teacher_scaled.sh"
)
...
if [[ ! -f "${project_root}/scripts/run_observation_uq_teacher_scaled.sh" ]]; then
