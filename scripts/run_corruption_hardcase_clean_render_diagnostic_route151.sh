...
# No-ORION A/B/C diagnosis for black-square artifacts in Route151 RGB capture.

project_root="${PROJECT_ROOT:-/public/home/lidachuan/project/Orion}"
asset_root="${ASSET_ROOT:-/public/share/lidachuan/orion_assets}"
run_id="${RUN_ID:-route151_clean_render_diagnostic_v1}"
output_root="${OUTPUT_ROOT:-${asset_root}/scenario_factory/corruption_hardcase_diagnostics/${run_id}}"
protocol="${project_root}/configs/scenario_factory/corruption_hardcase_clean_render_diagnostic_route151_v1.json"
...
port="${PORT:-$((24000 + port_offset))}"
tm_port="${TM_PORT:-$((44000 + port_offset))}"
server_log="${output_root}/logs/carla-server.log"

if [[ -e "${output_root}" ]]; then
  echo "[FAIL] refusing to reuse clean-render diagnostic root: ${output_root}" >&2
  exit 1
fi
for prerequisite in \
  ...
  "${project_root}/scripts/render_clean_camera_diagnostic.py"; do
  if [[ ! -f "${prerequisite}" ]]; then
    echo "[FAIL] missing clean-render diagnostic prerequisite: ${prerequisite}" >&2
    exit 2
  fi
done
...
export XDG_RUNTIME_DIR="/tmp/hardcase-clean-diagnostic-${USER:-unknown}-${job_tag}"
...
assert p["schema"] == "orion.clean_camera_render_diagnostic.v1"
assert [row["profile"] for row in p["conditions"]] == ["none", "intensity_zero_only", "clean"]
assert p["execution_locks"]["orion_loaded"] is False
assert p["execution_locks"]["orion_hardcase_screen"] is False
print("CLEAN_RENDER_DIAGNOSTIC_PREFLIGHT_OK=1")
PY

echo "RUN_ID=${run_id}"
echo "SCOPE=route151_clean_camera_blueprint_diagnostic_only"
echo "ORION_LOAD=0"
...
while (( SECONDS < deadline )); do
  if ! kill -0 "${server_pid}" 2>/dev/null; then
    echo "[FAIL] CARLA exited during clean-render diagnostic startup" >&2
...
if [[ "${ready}" != "1" ]]; then
  echo "[FAIL] CARLA clean-render diagnostic server did not become ready" >&2
  exit 1
fi

for profile in none intensity_zero_only clean; do
  ...
  if ! timeout --signal=TERM --kill-after=30s 360s env \
    ...
    RESUME=False \
    ALGO=hardcase_clean_diagnostic \
    PLANNER_TYPE=npc \
    ...
    echo "[FAIL] clean-render diagnostic failed for ${profile}" >&2
...
"${python_runner}" "${project_root}/scripts/render_clean_camera_diagnostic.py" \
  --root "${output_root}" \
  --protocol "${protocol}" \
...
sha256sum \
  "${project_root}/team_code/glare_capture_agent.py" \
  "${project_root}/team_code/orion_native_motion_blur.py" \
  "${project_root}/scripts/render_clean_camera_diagnostic.py" \
  "${protocol}" "${source_route}" \
  >"${output_root}/artifact_sha256.txt"
echo "CLEAN_RENDER_DIAGNOSTIC_OK=1"
