#!/usr/bin/env python3
"""Plan or submit the hash-bound Wave1 Q2 clean repeat jobs."""
...
SCHEMA = "orion.corruption_hardcase_wave1_clean_q2_submission.v1"
ACTIVATION_SCHEMA = "orion.corruption_hardcase_wave1_clean_q2_activation.v1"
PROTOCOL_SCHEMA = "orion.corruption_hardcase_wave1_clean_qualification.v1"
COLLECTION_SCHEMA = "orion.corruption_hardcase_clean_qualification_collection.v1"
CORRUPTION_KEYS = {
...
def build_jobs(
    *, activation: dict[str, Any], repository_root: Path, asset_root: Path
    ...
    if activation.get("schema") != ACTIVATION_SCHEMA:
        raise ValueError("unexpected Q2 activation schema")
    if activation["execution_authority"].get("q2_clean_submission") is not True:
        raise ValueError("Q2 clean submission is not authorized")
    protocol_path = repository_root / activation["base_protocol"]["path"]
    q1_result_path = repository_root / activation["q1_result"]["path"]
    collection_path = Path(activation["q1_collection"]["path"])
    for label, path, expected in (
        ("base protocol", protocol_path, activation["base_protocol"]["sha256"]),
        ("Q1 result", q1_result_path, activation["q1_result"]["sha256"]),
        ("Q1 collection", collection_path, activation["q1_collection"]["sha256"]),
    ):
        if not path.is_file():
            raise FileNotFoundError("missing %s: %s" % (label, path))
        if sha256(path) != expected:
            raise ValueError("%s hash differs" % label)
    protocol = read_json(protocol_path)
    collection = read_json(collection_path)
    if protocol.get("schema") != PROTOCOL_SCHEMA:
        raise ValueError("unexpected base protocol schema")
    if collection.get("schema") != COLLECTION_SCHEMA:
        raise ValueError("unexpected Q1 collection schema")
    routes = list(activation["scope"]["routes"])
    if routes != collection.get("q1_pass_routes"):
        raise ValueError("Q2 routes differ from exact Q1 passers")
    q1_result = read_json(q1_result_path)
    if routes != q1_result.get("decision", {}).get("q2_exact_scope"):
        raise ValueError("Q2 routes differ from frozen Q1 result")
    route_by_index = {int(row["route_index"]): row for row in protocol["routes"]}
    resource = activation["resources"]
    excluded = ",".join(sorted(set(resource["excluded_nodes"])))
    jobs = []
    for route_index in routes:
        route = route_by_index.get(route_index)
        if route is None:
            raise ValueError("Q2 route is absent from base protocol")
        ...
        environment = {
            ...
            "PILOT_RUN_ID": activation["scope"]["run_id"],
        ...
        if CORRUPTION_KEYS.intersection(environment):
            raise ValueError("Q2 clean repeat inherited corruption variables")
        jobs.append({
            "job_key": "route%d_clean_q2" % route_index,
...
def select_jobs(
    jobs: list[dict[str, Any]], route_indices: list[int]
) -> list[dict[str, Any]]:
    if not route_indices:
        return jobs
    requested = set(route_indices)
    known = {int(job["route_index"]) for job in jobs}
    if requested - known:
        raise ValueError("unknown Q2 route indices: %s" % sorted(requested - known))
    selected = [job for job in jobs if int(job["route_index"]) in requested]
    if len(selected) != len(requested):
        raise ValueError("duplicate Q2 route selection")
    return selected
...
def main() -> int:
    ...
    if args.output.exists():
        raise FileExistsError("refusing to overwrite Q2 submission record")
    ...
    jobs = select_jobs(jobs, args.route_index)
    ...
    if args.execute:
        ...
        submitter = args.repository_root.resolve() / "scripts/submit_closedloop_uq_pilot.sh"
        for job in jobs:
            try:
                completed = subprocess.run(
                    ["bash", str(submitter), str(job["route_index"]), "clean_off", "hazard"],
                    check=True,
                ...
                payload["job_ids"].append({
                    "job_key": job["job_key"], "job_id": int(job_id)
                })
